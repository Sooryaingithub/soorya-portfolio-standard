...
function getTheme(year: number) {
  if (year === 2026) {
    return { primary: "#8A2BE2", secondary: "#00F0FF" }; // Violet / Cyan
  } else if (year === 2025) {
    return { primary: "#10B981", secondary: "#06B6D4" }; // Emerald / Teal
  } else if (year === 2024) {
    return { primary: "#F59E0B", secondary: "#E11D48" }; // Amber / Rose
  }
  return { primary: "#8A2BE2", secondary: "#00F0FF" }; // Default
}
...
export default function ProjectPage() {
  ...
  const { primary, secondary } = getTheme(project.year);
  ...
  return (
    <main ref={containerRef} className="flex-1 bg-background relative selection:bg-white/20">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none transition-colors duration-1000">
        <div 
          className="absolute top-1/3 left-1/3 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] blur-[150px] rounded-full opacity-15"
          style={{ backgroundColor: primary }}
        />
        <div 
          className="absolute top-1/2 right-1/4 translate-x-1/4 -translate-y-1/2 w-[600px] h-[600px] blur-[150px] rounded-full opacity-15"
      ...
      {/* Hero Section */}
      <section className="relative h-[80dvh] flex flex-col items-center justify-center overflow-hidden z-10">
        <motion.div style={{ y: y1, opacity: opacity1 }} className="z-10 text-center px-4 max-w-4xl w-full">
          ...
          <motion.h1 
            ...
            className="text-5xl md:text-8xl font-semibold tracking-tighter mb-6 text-balance drop-shadow-2xl"
          >
            {project.title}
          </motion.h1>
          ...
          <motion.p 
            ...
            className="text-xl md:text-3xl font-medium tracking-tight text-foreground/60 text-balance mx-auto max-w-2xl mb-8"
          >
          ...
          >
            <span className="px-3 py-1 rounded-full text-xs font-semibold tracking-widest uppercase border" style={{ borderColor: `${primary}40`, color: primary, backgroundColor: `${primary}10` }}>
              {project.status}
            </span>
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="glass-panel p-6 md:p-8 flex flex-col md:flex-row gap-8 justify-between -mt-16 mb-24 relative z-30">
            <div>
              ...
              <p className="font-medium" style={{ color: primary }}>{project.status}</p>
            </div>
            <div>
                  ...
                  {project.content.capabilities.map((feature, idx) => {
                    ...
                    return (
                      <li key={idx} className="flex gap-4 p-4 glass-panel rounded-2xl">
                        <span className="mt-1 flex-shrink-0" style={{ color: secondary }}>•</span>
                        <span>
                          {parts.length > 1 ? (
                            <>
                  ...
                  <div className={`grid gap-8 mt-12 ${project.content?.images?.architecture && project.content?.images?.flowchart ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
                    ...
                    {project.content?.images?.flowchart && (
                      <div className="w-full aspect-video rounded-3xl bg-foreground/5 border border-foreground/10 flex items-center justify-center overflow-hidden relative group bg-white/5">
                        <div className="relative w-full h-full">
                          <Image src={project.content.images.flowchart} alt={`${project.title} Flowchart`} fill className="object-contain p-4" />
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-8 mt-12">
                    {project.content?.images?.ui && (
                      <div className="w-full aspect-[16/10] rounded-3xl bg-foreground/5 border border-foreground/10 flex items-center justify-center overflow-hidden relative group">
                        <div className="relative w-full h-full">
                          <Image src={project.content.images.ui} alt={`${project.title} Expected UI`} fill className="object-contain p-4" />
                        </div>
                      </div>
                    )}
                    {project.content?.images?.icons && (
                      <div className="w-full aspect-[21/9] rounded-3xl bg-foreground/5 border border-foreground/10 flex items-center justify-center overflow-hidden relative group bg-white/5">
                        <div className="relative w-full h-full">
                          <Image src={project.content.images.icons} alt={`${project.title} App Icons`} fill className="object-contain p-4" />
                        </div>
                      </div>
                    )}
                  </div>
